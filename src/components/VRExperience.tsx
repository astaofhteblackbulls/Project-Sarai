...
const VRExperience: React.FC<VRExperienceProps> = ({ destination, isOpen, onClose }) => {
      ...
      timer = setInterval(() => {
        setPreviewTime((prev) => {
          if (prev <= 1) {
            ...
            // Only restrict playback, don't hide the video
            if (videoRef) {
              // We no longer hide the video when preview expires
  ...
  }, [isOpen, hasAccess, previewTime, hasVideos, videoError, isVideoLoading, videoRef, isVideoPlaying, videoStarted, isPaused]);
  ...
  const handlePaymentSuccess = () => {
    ...
    // No need to re-enable video visibility since we're not hiding it
    // Just ensure pointer events are enabled for premium users
    if (videoRef) {
                ...
                {!videoError && currentVideo?.youtubeId && (
                  <>
                    ...
                    >
                      <iframe
                        ref={setVideoRef}
                        src={`https://www.youtube-nocookie.com/embed/${currentVideo.youtubeId}?enablejsapi=1&autoplay=0&controls=0&modestbranding=1&rel=0&showinfo=0&fs=0&cc_load_policy=0&iv_load_policy=3&origin=${window.location.origin}&disablekb=1&playsinline=1&loop=0&start=0${!hasAccess && isPreviewExpired ? '&end=60' : ''}`}
                      ...
                      {/* Secure overlay covering entire video */}
                      <div 
                      ...
                      {(!isVideoPlaying || isPaused) && !isPreviewExpired && (
                        <div 
                          ...
                          onClick={() => {
                            ...
                            setIsPaused(false);
                            // Try to start video playback by posting message to iframe
                        ...
                        <div className="absolute inset-0" style={{ zIndex: 25 }}>
                          <button
                            onClick={() => {
                              setIsPaused(true);
                              setIsVideoPlaying(false);
                              // Pause video by posting message to iframe
                              if (videoRef && videoRef.contentWindow) {
        ...
        <div className="fixed inset-0 bg-black z-[70] flex items-center justify-center animate-fade-in">
          ...
          <div className="w-full h-full max-w-none max-h-none p-6">
            <div className="relative w-full h-full">
              <iframe
                src={`https://www.youtube-nocookie.com/embed/${currentVideo.youtubeId}?autoplay=1&controls=1&modestbranding=1&rel=0&showinfo=0&fs=1&cc_load_policy=0&iv_load_policy=3&enablejsapi=0&origin=${window.location.origin}`}
                title={`${destination?.name} - ${currentVideo.title}`}
